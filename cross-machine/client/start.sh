...
echo ""
echo "[0/4] 清理残留进程 ..."
for PID_FILE in "${BASE_DIR}/.node_a_pid" "${BASE_DIR}/.node_c_pid" "${BASE_DIR}/.node_d_pid" "${BASE_DIR}/.node_e_pid"; do
    if [ -f "${PID_FILE}" ]; then
...
# [4/4] 启动节点 E (TCP 无协议)
echo ""
echo "[4/4] 启动节点 E (TCP 无协议客户端) ..."
cd "${BASE_DIR}/node_e"
${CKB_BIN} run -C "${BASE_DIR}/node_e" > "${BASE_DIR}/node_e.log" 2>&1 &
NODE_E_PID=$!
echo "${NODE_E_PID}" > "${BASE_DIR}/.node_e_pid"
echo "  节点 E PID: ${NODE_E_PID}"

# 等待 RPC 就绪
for i in $(seq 1 30); do
    if curl -s http://127.0.0.1:8154 -X POST -H 'Content-Type: application/json' \
        -d '{"id":1,"jsonrpc":"2.0","method":"local_node_info","params":[]}' >/dev/null 2>&1; then
        echo "  ✓ 节点 E RPC 就绪"
        break
    fi
    [ "$i" -eq 30 ] && echo "  ✗ 节点 E RPC 超时" && exit 1
    sleep 1
done
...
echo "  节点 E (TCP/无协议): PID=${NODE_E_PID}  RPC=127.0.0.1:8154"
