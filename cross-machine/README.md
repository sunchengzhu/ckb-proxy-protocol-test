...
```
  ...
  +---------------------+          +---------------------------+
  | 节点 A (TCP, P2P:8116)  ----TCP---->| HAProxy :18115            |
  |                     |          |   send-proxy-v2 -> :8115  |
  | 节点 C (WS,  P2P:8117)  ----WS---->| HAProxy :18080            |-->  节点 B
...
**网络要求:**
- 客户端能访问服务端的 **8114** (RPC)、**18115** (TCP 代理)、**18080** (WS 代理) 端口
- 如有防火墙，需放行这三个端口
...
| HAProxy TCP | 服务端 | 18115 | send-proxy-v2 -> :8115 |
| HAProxy WS | 服务端 | 18080 | X-Forwarded-For/Port -> :8115 |
...
| 检查 4b | TCP 端口 | 端口非 HAProxy 代理端口 18115 |
